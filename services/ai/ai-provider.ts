...
// Context for slot-specific suggestions
export type SlotContext = {
  targetCategory: string;
  currentOutfitItems: ItemSummary[];
...
// Context for natural language requests
export type NLContext = {
  prompt: string;
  currentOutfitItems: ItemSummary[];
  allClosetItems: ItemSummary[];
  userProfile?: UserProfile;
...
export interface AIProvider {
  ...
  suggestItemsForSlot(context: SlotContext): Promise<AISlotSuggestions>;

  // Suggest items to purchase for a specific category slot
  suggestPurchasesForSlot(context: SlotContext): Promise<AIPurchaseSuggestions>;

  // Process natural language outfit building request
  processNaturalLanguage(context: NLContext): Promise<AINaturalLanguageResponse>;
}
