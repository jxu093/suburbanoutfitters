...
import type { AIProvider, InspirationMatch, NLContext, OutfitContext, ShoppingContext, SlotContext } from './ai-provider';
...
// For local dev: set EXPO_PUBLIC_CLAUDE_API_KEY in .env.local
// For EAS builds: set EXPO_PUBLIC_CLAUDE_API_KEY in EAS secrets
const getEnvApiKey = () => process.env.EXPO_PUBLIC_CLAUDE_API_KEY;
...
class AIService {
  ...
  async initialize(): Promise<boolean> {
    ...
    const apiKey = (await getSetting(AI_API_KEY)) || getEnvApiKey();
  ...
  // Suggest items from closet for a specific category slot
  async suggestItemsForSlot(
    targetCategory: string,
    currentOutfitItems: Item[],
    candidateItems: Item[],
    occasion?: string,
    weather?: string
  ): Promise<AISlotSuggestions> {
    const provider = await this.ensureProvider();
    ...
    const context: SlotContext = {
      targetCategory,
      currentOutfitItems: currentOutfitItems.filter((item) => item.id).map(itemToSummary),
      candidateItems: candidateItems.filter((item) => item.id).map(itemToSummary),
      occasion,
      weather,
      userProfile: profile,
    };

    return provider.suggestItemsForSlot(context);
  }

  // Suggest items to purchase for a specific category slot
  async suggestPurchasesForSlot(
    targetCategory: string,
    currentOutfitItems: Item[],
    occasion?: string,
    weather?: string
  ): Promise<AIPurchaseSuggestions> {
    const provider = await this.ensureProvider();
    ...
    const context: SlotContext = {
      targetCategory,
      currentOutfitItems: currentOutfitItems.filter((item) => item.id).map(itemToSummary),
      candidateItems: [], // Not needed for purchase suggestions
      occasion,
      weather,
      userProfile: profile,
    };

    return provider.suggestPurchasesForSlot(context);
  }

  // Process a natural language outfit building request
  async processNaturalLanguageRequest(
    ...
    const context: NLContext = {
      prompt,
      currentOutfitItems: currentOutfitItems.filter((item) => item.id).map(itemToSummary),
      allClosetItems: allClosetItems.filter((item) => item.id).map(itemToSummary),
      userProfile: profile,
      conversationHistory,
    };

    return provider.processNaturalLanguage(context);
  }
}
