...
// Slot suggestion from AI - suggests items from closet for a specific slot
export type AISlotSuggestion = {
  itemId: number;
  reasoning: string;
  matchScore: number; // 0-1 how well this item fits
};

// Response from AI slot suggestion
export type AISlotSuggestions = {
  suggestions: AISlotSuggestion[];
  advice?: string;
};

// Purchase suggestion for a slot (simplified - no price focus)
export type AIPurchaseSuggestion = {
  category: string;
  description: string;
  colors?: string[];
  style?: string[];
...
// Response from AI purchase suggestion for slot
export type AIPurchaseSuggestions = {
  suggestions: AIPurchaseSuggestion[];
  advice?: string;
};

// Natural language request context
...
// Response from natural language outfit builder
export type AINaturalLanguageResponse = {
  message: string;
  suggestedClosetItemIds?: number[];
  closetReasonings?: { [itemId: number]: string };
  purchaseSuggestions?: AIPurchaseSuggestion[];
